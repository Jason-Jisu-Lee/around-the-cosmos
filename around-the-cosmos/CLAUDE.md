...
> Phase-varying payout is a **default feature** of the moon (no upgrade): it visibly waxes/wanes on a
> `MOON_CYCLE`=19.2s loop and payout rides the cycle — ×1 at new moon up to ×1.5 at full moon (pure upside).
> (The moon has **no unique upgrade** for now — only Payout + Speed — since Lunar Phases became the default.)
...
> own Payout (+amount/lvl) and Speed upgrade. The asteroid's unique upgrade is **Composition** (`astcomp`); the moon has no unique upgrade (its **phase-varying payout is a default feature** — `moonPhaseMult()`, ×1→×1.5 over the cycle).
>
...
- **Orbiters / payout**: three clumps (dust ring 0, asteroid ring 1, moon ring 2), rendered by the shared `drawClump()` helper (render.js).
  ...
  - **Moon** (single body) — `G.moons[]`, clump on `G.moonClump` (ring 2, period `PLANET_DEF[2].period`=13s — the slowest). **Sits exactly on the orbit line** (`localR:0` — no local wobble, unlike dust/asteroid). Speed = `moonSpeed()`=`0.78 × moonspd.mult(lvl)` (**0.78 base factor** → effective 78% → 156%). On top-cross pays `moonPayout()` = `round((200 + 200·lvl(moonpay)) × moonPhaseMult() × resonanceMult())` (rounded → integer; in orbiters/moon.js). Largest body, radius `(PLANET_DEF[2].radius/3+6)×1.8`, color `#8f9498`. **Rendered by `drawRoundClump`/`drawMoonDisc` (render.js), not `drawClump`** (`o.round:true`): a near-circular disc whose rim is **subtly perturbed by low-freq ridges** (`MOON_RIDGES`, ~1–2% of r — perfect at a glance, lumpier up close) with faint darker **maria/craters** (`MOON_CRATERS`, fixed → a tidally-locked face), built by the `moonOutline()` helper. Over it a **lunar-phase terminator shadow** (`MOON_SHADOW` `#3b3f44`, drawn at r×1.05 so it reaches the ridged rim) waxes/wanes on the `MOON_CYCLE`=19.2s loop (`moonPhase()`/`moonLit()`), and a **soft limb-darkening rim shadow** (radial gradient, only the outer ~20%) rounds the flat disc into a 3D sphere. **Payout rides the phase by default** via `moonPhaseMult()`=1+0.5·moonLit() (×1 new → ×1.5 full); the observatory uses `moonAvgPayout()` (×1.25 mean) so stats don't flicker. No motes.
...
- **Cosmic info** (`ui/cosmo.js` + the orbiters registry): `cosmoTargetAt(x,y)` returns `'comet'` (within `COMET_HOVER_R`=40px), `'lacuna'` (22px), or an **orbiter id** by iterating `ORBITERS` (each entry's `clumpPos()`/`hoverR`). Orbiter card titles/rows/descriptions come from `ORBITER_BY_ID[id]`; the Lacuna card is built inline (`lacunaRows()` + `LACUNA_DESC`). The asteroid card includes a Composition (tier name) row. The **moon card** is explicitly a **small moon**: it shows physical stats (Class "Small moon" / Diameter 80 km / Mass / Surface gravity, from `PHYS.moonRadius`=40 km + `PHYS.moonDensity`, via `moonBodyMass()`/`moonGravity()` in orbiters/moon.js) plus gameplay rows (Phase / Orbit payout / Orbits per hour). "Small moon" is called out deliberately — the next planned body is a **dwarf planet**, and some real moons exceed dwarf planets, so the size order is kept sane.
