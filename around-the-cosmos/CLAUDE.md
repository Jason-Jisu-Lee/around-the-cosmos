...
| **`ui/`** | `panels.js` (upgrade accordion + visibleSig), `observatory.js` (stats DOM + `updateObservatory`), `hud.js` (`updateUI` orchestrator), `cosmo.js` (hover tooltip + click-to-pin info cards, comet reticle label), `accretion.js` (**prestige screen** — Tabs layout overlay; `openAccretion`/`closeAccretion`, category tabs Lacuna/Orbiters/Phenomena/Cycles, placeholder tree; the **Accrete** button opens `#acc-confirm`, a plain-language modal explaining the **Mass** gain + reset, whose confirm calls `doAccretion()`. Real per-category upgrades still TBD) |
...
**Hide completed:** maxed upgrades are **hidden by default**. The "Hide completed" toggle
...
## Accretion (prestige) — partial: entry + Mass grant/reset wired, upgrades TBD
- **`#accretion-btn`** (bottom-left, Dusty-Blue accent) is **hidden until lifetime `G.totalDust` first passes `ACCRETION_THRESHOLD`=200,000** (`canAccrete()`; `hud.js` toggles display via an explicit `'block'` — `''` would fall back to the CSS `display:none`). Once passed it stays (totalDust never decreases).
- It opens **`#accretion-screen`** (Tabs overlay, `ui/accretion.js`). The screen's **Accrete** button opens **`#acc-confirm`** — a plain-language modal that shows the Mass gain + what resets/keeps.
- **Mass** (`G.mass`, the meta-currency): `massEarnable()` = `floor(3 × √(totalDust / 200000))` (200k→3, 800k→6, 1.8M→9, 5M→15), based on **all-time stardust**; a grant = `massGain()` = `massEarnable() − G.massEarned` (anti-farm: re-earning the same total adds ~0). `doAccretion()` (state.js) adds the gain to `mass`+`massEarned`, then **full-resets** via `createInitialState()` **keeping only** `mass`/`massEarned`/`totalDust`/`gameTime`, and rebuilds panels. Spending Mass on real per-category upgrades is still TBD (placeholder tree).
