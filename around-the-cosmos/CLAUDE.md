...
3. `runAccretionFx()` plays **Implosion & Rebirth** on the full-window `#accretion-fx` canvas (z110, **opaque** — covers the game; centered on the on-screen Lacuna): snapshots the live orbiters (`o.clumpPos()`/`o.color()`), parchment darkens to space, orbiters + stars rush in, white flash + gold shockwave, then a new universe glows.
