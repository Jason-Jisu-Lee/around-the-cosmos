...
- **Cosmic info card** (`#cosmo-tip`, built in `game.js`): hovering the **Lacuna** (within 22px of center) or the **dust clump** (within 46px of `clumpPos()` — generous, since the clump is small and moving) opens an info card **pinned in the center of the sky** (not following the cursor). The card is **sticky** — it stays after the cursor leaves so you can read it — and is dismissed with the **× button** (top-right) or **Escape** (`closeCosmoCard`). Hovering the card itself sets `cosmoOver=false` (via the card's `mouseenter`) so the open card never flips while you reach for ×; a `dismissedTarget` guard stops the just-closed card from instantly reopening until the cursor leaves that target. Card is `pointer-events:auto` (a light modal over the sky center).
  - **Lacuna card:** Diameter / Mass / Surface gravity / Escape velocity / Density + a one-sentence flavor line (`LACUNA_DESC`, the protagonist). **Dust Particle card:** Orbit payout / Orbital speed / Orbits-per-hour + flavor line (`ORBITER_DESC`).
  - All numbers derive from `PHYS` (config.js) via `state.js` physics helpers. Display uses **`fmtNice`** (≤2 decimals, integer ≥100) with a chosen unit so values read cleanly — surface gravity in **cm/s²** (8.39), escape velocity in **m/s** (142); mass uses **`fmtSci`** (1.81 × 10¹⁹ kg). Orbital speed and orbits/hour **scale with the Dust Particle Speed upgrade** (via `dustSpeed()`), so buying Speed visibly raises them. innerHTML only rewrites when content changes (`cosmoTip._html` cache). Base model: Lacuna r=120 km, ρ=2.50 g/cm³ → M≈1.81×10¹⁹ kg, g≈8.39 cm/s², v_esc≈142 m/s; orbiter at r=200 km → v≈77.7 m/s, ≈0.22 orbits/hr (at base speed). Future science-based upgrades scale `PHYS` and everything recomputes.
